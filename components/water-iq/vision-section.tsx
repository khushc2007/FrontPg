...
const milestones = [
  { year: "2024", label: "WATER-IQ Prototype",  desc: "First 5-stage treatment system validated", active: true },
  { year: "2025", label: "Pilot Deployments",   desc: "12 apartment buildings in metro areas",   active: true },
  { year: "2026", label: "Platform Launch",     desc: "City-scale rollout with partner buildings", active: true },
  { year: "2030", label: "Sustainable Cities",  desc: "1M+ liters reused daily across the network", active: false },
]
  ...
  return (
    <section id="vision" ref={containerRef} className="relative py-36 px-6 overflow-hidden">
      ...
      <div ref={ref} className="relative z-10 mx-auto max-w-5xl">
        ...
        <ScrollReveal delay={0.3} className="mt-12 flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
          <GlowButton href="#solution" variant="primary" className="rounded-2xl px-10 py-4">
            Start the Journey
            <ArrowRight className="h-4 w-4" />
          </GlowButton>
          <GlowButton href="#problem" variant="secondary" className="rounded-2xl px-10 py-4">
        ...
        {/* Roadmap timeline */}
        <ScrollReveal delay={0.4} className="mt-28">
          ...
          <div className="relative">
            ...
            <StaggerReveal className="grid grid-cols-2 gap-8 md:grid-cols-4" baseDelay={0.6}>
              {milestones.map((m, i) => (
                <div key={m.year} className="relative pt-10 text-center">
                  <motion.div
                    className="absolute top-3.5 left-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full border-2"
                    style={{
                      borderColor: m.active ? "#00D4FF" : "#1A2E4A",
                      backgroundColor: m.active ? "#00D4FF" : "#0A1628",
                      boxShadow: m.active ? "0 0 12px rgba(0,212,255,0.5)" : "none",
                    }}
                    animate={m.active ? { boxShadow: ["0 0 8px rgba(0,212,255,0.3)", "0 0 20px rgba(0,212,255,0.6)", "0 0 8px rgba(0,212,255,0.3)"] } : {}}
